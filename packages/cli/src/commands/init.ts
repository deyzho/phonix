/**
 * phonix init — interactive project setup.
 *
 * Steps:
...
const BLANK_INDEX_TS = `// phonix deployment script
...
export async function runInit(cwd: string = process.cwd()): Promise<void> {
  ...
  console.log();
  console.log(chalk.bold.cyan('  Welcome to Phonix SDK v0.1'));
  console.log(chalk.gray('  Build edge dApps once. Run them on millions of phones.\n'));

  // Check if phonix.json already exists
    ...
    const { overwrite } = await inquirer.prompt<{ overwrite: boolean }>([
      {
        type: 'confirm',
        name: 'overwrite',
        message: chalk.yellow('phonix.json already exists. Overwrite?'),
        default: false,
      },
    ]);
    if (!overwrite) {
  ...
  try {
    // ── Generate and write phonix.json ─────────────────────────────────────
    ...
    const envContent =
      ...
      `# Run: phonix auth ${answers.provider}   to set up credentials interactively.\n` +
      `#\n` +
      `# P256 private key — auto-generated by phonix init\n` +
      `PHONIX_SECRET_KEY=${secretKeyHex}\n`;
    ...
    if (!setupNow) {
      ...
      console.log(chalk.bold.white(`       phonix auth ${answers.provider}`));
      console.log();
      console.log(
      ...
      console.log(chalk.bold.white('       phonix deploy'));
    } else {
      console.log(
      ...
      console.log(chalk.bold.white('       phonix run-local'));
      console.log();
      console.log(`  ${chalk.cyan('3.')} Deploy:`);
      console.log(chalk.bold.white('       phonix deploy'));
    }
    ...
    console.log(chalk.bold.white('       phonix send <processorId> \'{"hello":"world"}\''));
    console.log();
    console.log(
      chalk.gray(
        `  Docs: https://github.com/phonix-sdk/phonix#readme`
      )
    );
    console.log();
  } catch (err) {
